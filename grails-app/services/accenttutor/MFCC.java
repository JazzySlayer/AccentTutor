...
//            throw new IllegalArgumentException("window size must be 2^n");
    ...
    normalizedPowerFFT = new FFT(FFT.FFT_NORMALIZED_POWER, windowSize, FFT.WND_HAMMING);
...
private double[] preEmphasis(double inputSignal[]) {
          ...
          for (int n = 1; n < inputSignal.length; n++) {
                  outputSignal[n] = (float) (inputSignal[n] - 0.97 * inputSignal[n - 1]);
          }
          return outputSignal;
  }
...
//  Actual windowSize and hopSize
  int newWindowSize = FeatureFileExtractor.newWindowSize; //25.625ms
  int hopsize = FeatureFileExtractor.hopsize; // 10ms
  
  // create an array of the size of the size of total no of samples in the given wavFile.
  double wavFile[] = new double[(int) ais.getFrameLength()];
  
  //Read the entire wavFile and store in the array wavFile
  @SuppressWarnings("unused")
  int samplesRead = in.append(wavFile, 0,(int) ais.getFrameLength());
  
  //making the size of array multiples of window length.
  int totalLength = wavFile.length - 2 * hopsize + newWindowSize;
  
  // copying the original wavfile to compute Energy
  double[] fullWavFile_orig = new double[totalLength]; 
  for(int i = 0; i < wavFile.length; i++)
	  fullWavFile_orig[i] = wavFile[i] * scale;
 
  //Pre-Emphasis on the whole wav file
  wavFile = preEmphasis(wavFile);
  
//  int totalLength = wavFile.length + newWindowSize - wavFile.length % hopsize;
  double[] fullWavFile_PE = new double[totalLength];
  
  //Scaling the values and appending zeros
  for(int i = 0; i < wavFile.length; i++)
	  fullWavFile_PE[i] = wavFile[i];
  
  double newInputData[] = new double[newWindowSize];
  double inputDataEnergy[] = new double[newWindowSize];
  double adjustInputData[] = new double[windowSize];
  double energy;
  int n = 0;
  while((n + newWindowSize) < totalLength){
	  //selecting a window
	  //newInputData = Arrays.copyOfRange(fullWavFile, n,	n + newWindowSize);
	  newInputData = Arrays.copyOfRange(fullWavFile_PE, n,	n + newWindowSize);
	  inputDataEnergy = Arrays.copyOfRange(fullWavFile_orig, n,	n + newWindowSize);
	  energy = calcEnergy(inputDataEnergy);
	  // windowing the input signal by hamming
	  newInputData = normalizedPowerFFT.applyWindowFunction(newInputData);
	  //appending the data with zeros to make it compatible with the program
	  for(int i = 0; i < newWindowSize; i++)
		  adjustInputData[i] = newInputData[i];
	  
	  inputData = adjustInputData;
	  //process thw window
	  mfcc.add(processWindow(inputData, 0,energy));
	  // move the window
	  n += hopsize;	  	  
  }
   ...
   * performed: <br>
   * <br>
   * (1) normalized power fft with hamming window function<br>
   ...
   * @throws IllegalArgumentException raised if method contract is violated
   */
  public double[] processWindow(double[] window, int start,double energy) throws IllegalArgumentException
  {
  ...
  public double calcEnergy(double frame[]) {
	  double sum = 0;
