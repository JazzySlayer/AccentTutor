 ...
 * output stream can be specified. The output stream has only one channel. If the
...
public class AudioPreProcessor
{
  ...
  */
   ...
   * easy access to the audio data. The audio samples of the output stream are 
   * in the range of [-1, 1]. 
   *
  ...
  public AudioPreProcessor(AudioInputStream in, float sampleRate) throws IllegalArgumentException
  {
    ...
    //check that sample rate is an integer value 
    if(Math.abs((Math.floor(sampleRate) - sampleRate)) > 0)
    ...
    //bits - 1 because positive and negative values are possible
    this.scale = 1.0 / ((double) converted.getChannels() * (1 << (converted.getSampleSizeInBits() - 1)));
  }
   ...
   * @see #fastSkip(int)
   *
   * @param len number of samples to skip, must be a positive value
   ...
   * skipped. The real number of skipped bytes will be returned.
   *
   * @param len number of samples to skip, must be a positive value
   * @return the number of samples actually skipped within the input stream
   * @throws IllegalArgumentException raised if method contract is violated
  ...
  public double[] get(int len) throws IllegalArgumentException, IOException
  {
    ...
    convertToSample(inputBuffer, bytesRead, outputData, 0);
  ...
  public int append(double[] buffer, int start, int len) throws IllegalArgumentException, IOException
  {
    ...
    //now append new data
    convertToSample(inputBuffer, bytesRead, buffer, start);
  ...
  /**
   * This method is internally used to convert the bytes of the input byte stream to
   * a steam of float pointing samples. The samples are in the range of [-1, 1].
    ...
    else
    {
      ...
      if(isBigEndian)
      {
        ...
        for (int i = 1, j = frameSize; i < len; i++)
        {
          ...
          {
            //convert into a single sample and re-scale in the range of [-1, 1] 
   ...
   * @throws IllegalArgumentException raised if method contract is violated or
...
static public AudioInputStream convertToPCM(AudioInputStream in) throws IllegalArgumentException
   ...
   * with the given sample rate. It might be useful to convert the audio
   * stream to PCM_SIGNED first, since your audio system might not support
   ...
   * stream with the given number of channels. It might be useful to convert
   * the audio stream to PCM_SIGNED first, since your audio system might not
  ...
  * bits to represent one sample. It might be useful to convert the audio
  * stream to PCM_SIGNED first, since your audio system might not support the
  ...
  * stream using the given byte order (big/little Endian). It might be useful
...
public static AudioPreProcessor getAudioPreProcessor(File input, float sampleRate) throws UnsupportedAudioFileException, IOException
  {
	  AudioInputStream ain;
		
      try
      {
     	 ain = new MPlayerAudioInputStream(input, sampleRate);
      }
      catch(Exception e)
      {
     	 ain = AudioSystem.getAudioInputStream(input);	 
      }  
      
      return new AudioPreProcessor(ain, sampleRate);
  }
   ...
   * not been reached. If the end of the stream has already been reached, the
